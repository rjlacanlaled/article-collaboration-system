  ...
  return (
    <>  
      <div  className='mr-4'>
        ...
        <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleCloseMenu} sx={{mt: 2, height: 865,}}>
          {NotificationData.map((notifications) => ( 
            <div className='w-500 border-1 rounded-md m-1.5 flex flex-col hover:bg-gray-100 cursor-pointer relative' onClick={handleCloseMenu}>
              ...
              <div className='flex justify-start items-center py-2'>
                <p className='self-start ml-7 mr-0.5 text-sm'>{notifications.title}</p>
              ...
              <h2 className='w-fit p-2 mx-5 font-bold'>{notifications.message}</h2>
              <p className='self-end px-1.5 py-2 mr-2 text-xs text-gray-600'>{notifications.posted}</p>
            </div>
          ))}
        </Menu>
      </div>
